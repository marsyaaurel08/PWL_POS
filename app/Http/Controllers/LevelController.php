...
class LevelController extends Controller
{
   ...
   /* public function create()
    ...
    public function update_ajax(Request $request, $id)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            $rules = [
                'level_kode' => 'required|min:3|unique:m_level,level_kode,' . $id . ',level_id',
            ...
            if ($check) {
                if (!$request->filled('level_kode')) { // jika level kode tidak diisi, maka hapus dari request
     ...
     //menampilkan detail data level dengan ajax
...
public function import()
    ...
    public function destroy(string $id)
    {
        $check = LevelModel::find($id);
        if (!$check) {
            return redirect('/level')->with('error', 'Data level berhasil dihapus');
        }
        try {
